...
#   ggsave( filename = paste0( "synComparison", cohortNames[i], ".pdf" ), plot = myPlot, width = 4, height = 5, units = 'in' )
  ...
  myPlot2 <- ggplot( plot2Data, aes( x = Volumes, y = Difference ) ) +
															...
															geom_hline( yintercept = 0, colour="navyblue", linetype = "solid", size = 1 ) +
															geom_smooth( method = lm, formula = y ~ poly( x, 3 ), se = FALSE, fullrange = TRUE, colour = "black", size = 1, linetype = "dashed" ) +  # 															geom_text(aes(x = 1000, y = -0.3, label = lm_eqn(lm(Difference ~ Volumes^2, plot2Data))), parse = TRUE) +
															scale_x_continuous( expression( paste( "Label volume (mm"^"3", ")" ) ) ) +
