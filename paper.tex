...
{\color{blue} guaranteed parametric continuity}.  Current
...
\footnote{
{\color{blue}{This phenomenon is explained in greater detail in \cite{tustison2009}
...
registration algorithms and provides {\color{blue} alternative smoothing 
...
\footnote{
{\color{blue}{For comparison, FFD image registration uses the
...
framework refactoring.  {\color{blue}{Much of the original ITK image registration infrastructure 
...
\footnote{
{\color{blue}{We gratefully acknowledge Marius Staring's contribution in providing the broad
...
\footnote{
{\color{blue}{This step corresponds to the {\tt --initial-moving-transform} option.  Generally, the
...
{\color{blue}{Table 1 summarizes core information about the data sets used.}}
...
\begin{table*}[htb]
  \caption{
    {\color{blue}{ Brief overview of the data sets used for the SyN vs. B-spline SyN comparison.}}
    }
  \centering
...
shape/intensity template.  {\color{blue}{Not only does this reduce the computational
...
for evaluation.%
\footnote{
{\color{blue}{The MALF labels are available at http://figshare.com/account/projects/196.}}
}
...
\begin{table*}[htb]
  \caption{
    {\color{blue}{Timing (in hours) per registration for both SyN algorithms across data sets.}}
    }
  \centering
...
The {\color{blue}{only}} difference between the two {\color{blue}{registration 
settings}} consist of the Gaussian and 
...
\begin{figure}[htb]
  ...
  SyN and B-spline SyN.  Note that, at least for these slices, the cortical 
  differences are the most prominent. 
  }
...
intended scope.  However, we note that in addition to this work,
...
convolution.  {\color{blue}{Also, we emphasize that underlying the DMFFD approaches 
