...
Equation (\ref{eq:dmffd}) for updating the time-varying velocity field control point lattice
is as follows:
...
The class {\tt itk::BSplineScatteredDataPointSetToImageFilter} underlies all DMFFD regularization which implements the 
methods described in \cite{tustison2009}.  Although applicable to
various scenarios (e.g. curve and surface estimation), it has
been optimized for imaging applications and multi-threaded
for fast processing on multi-core machines.
...
\footnote{
The long command and short command line help menus can be invoked by
the commands `{\tt antsRegistrationantsRegistration --help}' and `{\tt antsRegistration -h}', respectively.
}
...
antsRegistration --dimensionality 3 \
                 ...
                 --smoothing-sigmas 2.0x1.0x0.0mm \
                 --shrink-factors 4x3x2 \
                 --transform BSplineSyN[0.1,10x10x10,0x0x0,3] \
...
criteria (including number of iterations perl resolution level) and metric (or
...
This was to minimize the obvious {\it observer dimensionality bias} where manual
...
stages were identical between the two algorithms).  Note that parameter
exploration is not a part of this work.  Rather, we rely on previously
...
data sets used in this study, times for B-spline SyN are approximately 
15-40\% greater than Gaussian-based SyN (single-threaded).
...
least for these data (which conforms with our experience extended to other
...
\footnote{
 It should be noted that the current {\tt antsRegistration} command line currently 
 permits specification of the
 B-spline total and update field smoothing in terms of total mesh size over
...
20 mm for a final knot spacing of 2.5 mm.  
...
we also accounted for label volumetric considerations \cite{rohlfing2012}.  In Figure
...
calculated the log of the Jacobian of the transformations from 
...
that B-spline SyN produces a much greater range of log Jacobian
...
% What about sparsely sampled data and the advantages of b-splines
